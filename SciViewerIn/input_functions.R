...
seurat2.signatures <- function(sa,signature_file=NULL,my_assay='RNA'){
...
seurat2sqlite <- function(so=NULL,si_study=NULL,
  ...
  si_study$donor <- ifelse(is.null(si_donor),NA, si_donor)
...
write.scstudy2.sqlitedb <- function(so,
  ...
  if(is.null(Celltype)){
    ...
    so@meta.data <- local({
      ...
      #names(meta) <- ifelse(names(meta)==Celltype,as.character("SVcell_type"),as.character(names(meta)))
      meta
    })
  }
